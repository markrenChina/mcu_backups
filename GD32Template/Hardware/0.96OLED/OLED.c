...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	GPIO_WriteBit(OLED_SCL_GPIO, OLED_SCL_PIN, (BitAction)BitValue);
	//gpio_bit_write(GPIOB,GPIO_PIN_8,(FlagStatus)BitValue);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	GPIO_WriteBit(OLED_SDA_GPIO, OLED_SDA_PIN, (BitAction)BitValue);
	//gpio_bit_write(GPIOB,GPIO_PIN_9,(FlagStatus)BitValue);
	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/
	//...
}
...
void OLED_GPIO_Init(void)
{
	...
	/*将SCL和SDA引脚初始化为开漏模式*/
	rcu_periph_clock_enable(OLED_SCL_RCC);
	rcu_periph_clock_enable(OLED_SDA_RCC);
	gpio_mode_set(OLED_SCL_GPIO, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, OLED_SCL_PIN);
	gpio_mode_set(OLED_SDA_GPIO, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, OLED_SDA_PIN);
	gpio_output_options_set(OLED_SCL_GPIO, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ, OLED_SCL_PIN);
	gpio_output_options_set(OLED_SDA_GPIO, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ, OLED_SDA_PIN);
